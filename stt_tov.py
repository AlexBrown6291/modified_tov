...
data = np.loadtxt("EOS_files/STT_EOS2.dat",skiprows=1,delimiter='\t')
data = data.transpose()
...
eos_rhog = eos_rhos * G * c**(-4.)          #Use G * c**-2 because this is mass density
...
for x in pressures:
    ...
    rho_c = EOS_geometric_fromfile(x)           #the thesis uses rho_c for both pressure and density scaling
    p_c = rho_c
