...
K_nr = (hbar**2./(15*np.pi**2. * m_e))*  ((3*np.pi**2.)/(m_h * c**2.))**(5./3.)
K_nr_g = K_nr *G**(-2./3.) * c**(5./3.)
#K = K_nr_g
...
for x in p:
    ...
    soln = solve_ivp(TOV,t_span,y0,method='RK45',events=star_boundary,dense_output=True, atol=10**(-10.))
...
plt.xlabel("radius (m )")
plt.ylabel("Mass (M_sun)")
plt.savefig("plots/mass_radius_one.pdf")
